...
class Mnu_Title
	...
	def initialize(vp)
		...
		@data = ['New','Load','Options','Quit']
		@strings = ["New Game","Load Game","Options","Quit"]
		...
		@data.each{ |item|
			...
			btn = Box.new(vp,240,45)
	     	...
	     	text.gradient = true
	     	text.text = @strings[@data.index(item)]
	     	text.move(cx+94,cy+7)
	     	@texts.push(text)

	     	icon = Sprite.new(vp)
	     	icon.bitmap = $cache.menu("Icons/"+item)
	     	icon.move(cx-4+28,cy-12)
